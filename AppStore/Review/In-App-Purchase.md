# SecondClock Pro（アプリ内課金）

## 商品情報

- 種類：非消耗型（Non-Consumable）
- Reference Name：`SecondClock Pro Lifetime`
- Product ID：`com.irochi.SecondClock.pro.lifetime`
- 初期価格案：日本700円相当の価格ポイント
- Family Sharing：オフ（初期案）

## App Store表示情報

- 表示名：`SecondClock Pro`
- 説明：`写真背景と限定テーマを買い切りで解放`
- 英語表示名：`SecondClock Pro`
- 英語説明：`Unlock photo backgrounds and exclusive themes.`

## Review Notes

SecondClock Pro is a one-time, non-consumable purchase. No subscription is used.

Access steps:
1. Launch the app.
2. Tap the gear button in the upper-right corner.
3. Tap the “SecondClock Pro” card.
4. The purchase button and “購入を復元” (Restore Purchases) button are shown on the paywall.

Pro unlocks photo backgrounds, photo dimming controls, three exclusive themes (Sunset, Sakura, and Night Sky), and photo backgrounds on Home Screen widgets.

No account or sign-in is required. The review screenshot is `AppStore/Review/iap-review.png`.

## 初回提出

初回のアプリ内課金は、アプリのバージョン1.0.0と同時に審査へ追加します。StoreKit商品IDはソースコードとStoreKit設定ファイルに実装済みです。
